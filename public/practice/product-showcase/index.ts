...
function initializeShowcase(): void {
  const filters: Array<[string, Product['type'] | undefined]> = [
    ['all', undefined], ['books', 'book'], ['electronics', 'electronics'], ['clothing', 'clothing'],
  ];
  for (const [id, filter] of filters) {
    const button = document.getElementById(id);
    if (button) button.onclick = () => showProducts(filter);
  }
  showProducts();
}

document.addEventListener('DOMContentLoaded', initializeShowcase);
if (document.readyState !== 'loading') initializeShowcase();
